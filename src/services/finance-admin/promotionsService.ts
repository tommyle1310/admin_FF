...
export const promotionsService = {
    getAllPromotions: async () => {
        const response = await axiosInstance.get(`finance-admin${API_ENDPOINTS.PROMOTIONS}`);
        return response.data;
    },

    createPromotion: async (promotion: Omit<Promotion, 'id'>) => {
        const response = await axiosInstance.post(`finance-admin${API_ENDPOINTS.PROMOTIONS}`, promotion);
        return response.data;
    },
